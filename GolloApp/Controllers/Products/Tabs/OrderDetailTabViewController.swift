...
class OrderDetailTabViewController: UIViewController {
    ...
    init(viewModel: OrderDetailTabViewModel, orderId: String, fromNotifications: Bool, idJob: String? = nil) {
    ...
    fileprivate func configureRx() {
        ...
        orderStatusButton
            ...
            .tap
            .subscribe(onNext: {
//                let url = "https://xandar-lsw-v3.instaleap.io/?job=\(self.idJob ?? "")&token=hkv6MpWZw6DwIPfuTlq0k3qNSX2JQkOxmYnOsPrO&language=es-ES&hideHelpCenter=true&hideProducts=true&hidePaymentMethod=true&hideOrderDetail=true&hideComments=true&primaryColor=%23005da4&hideOrderId=true&hideCancelOrder=true"
//                self.openUrl(url)
                let webViewOrderStatusViewController = WebViewOrderStatusViewController(idJob: self.idJob)
                webViewOrderStatusViewController.modalPresentationStyle = .fullScreen
                self.navigationController?.pushViewController(webViewOrderStatusViewController, animated: true)
            })
            .disposed(by: bag)
    }

    func configureTableView() {
        orderStatusButton.layer.cornerRadius = 10
        orderStatusButton.layer.masksToBounds = true
        orderStatusButton.layoutIfNeeded()
    ...
    func showData(with order: OrderDetailData) {
        ...
        if deliveryPlace.tipoEntrega == "20" {
            ...
            if let idJob = idJob, !idJob.isEmpty {
                orderStatusButton.isHidden = false
            } else {
                orderStatusButton.isHidden = true
            }
        } else {
