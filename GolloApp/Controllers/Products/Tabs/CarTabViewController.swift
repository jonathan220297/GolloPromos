...
class CarTabViewController: UIViewController {
    ...
    func checkIfUserRegistered() {
        if Variables.isRegisterUser {
            let paymentTypeViewController = PaymentTypeViewController(delegate: self)
            paymentTypeViewController.modalPresentationStyle = .overCurrentContext
            paymentTypeViewController.modalTransitionStyle = .crossDissolve
            self.present(paymentTypeViewController, animated: true)
        } else {
...
extension CarTabViewController: PaymentTypeDelegate {
