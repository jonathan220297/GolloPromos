//
//  UIColor+Assets.swift
//  PromosGollo
//
//  Created by Rodrigo Osegueda on 30/8/21.
//

import UIKit

extension UIColor {
    static let terracotta = UIColor.fromAsset(named: "Terracotta")
    static let primary = UIColor.fromAsset(named: "colorPrimary")
    static let primaryLight = UIColor.fromAsset(named: "colorPrimaryLight")
    static let gift = UIColor.fromAsset(named: "colorGift")
    static let bonus = UIColor.fromAsset(named: "colorBonus")
    static let discount = UIColor.fromAsset(named: "colorDiscount")
    static let grayPrimary = UIColor.fromAsset(named: "menuGray")
    static let blueLight = UIColor.fromAsset(named: "blueLight")
    static let yellow = UIColor.fromAsset(named: "yellow")
    static let backgroundGray = UIColor.fromAsset(named: "backgroundGray")
    static let disableGray = UIColor.fromAsset(named: "disableGray")
}

fileprivate extension UIColor {
    static func fromAsset(named name: String) -> UIColor {
        guard let color = UIColor(named: name) else {
            fatalError("Color \(name) is not defined in a color asset.")
        }
        return color
    }
}

